...
export const useUpdateForceDevelopmentAnalysis = (clientId: number, ForceDevelopmentAnalysisId: number) => {
  const UpdateForceDevelopmentAnalysis = useCallback(async (data: any) => {
    const response = await api.post(
      `/clients/questionnaires/force-development-analysis/${ForceDevelopmentAnalysisId}/update`,
      data,
      );
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries([clientId, "force-development-analysis", ForceDevelopmentAnalysisId ]);
    },
  });
};
