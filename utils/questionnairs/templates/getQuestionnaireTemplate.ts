...
export const getQuestionnairesTemplate = async ({ questionnairesId, templateType }: QuestionnairesTemplateParams) => {
    ...
    try {
        const response = await api.post<QuestionnairesTemplateResDto>(`/clients/questionnaires/generate-document-link`, { ids: questionnairesId, type: templateType }, {
            timeout: 540000,
        });
        return response.data;
    } catch (error) {
