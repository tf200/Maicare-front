...
async function uploadFile(file: File, endpoint: UploadEndpointType = "global") {
  ...
  const response = await api.post<FileUploadResponse>(endpoints[endpoint], formData, {
    headers: {
...
async function patchFileData(fileId: string, data: Partial<FileUploadResponse>) {
...
export const usePatchFileData = (fileId: string) => {
  const queryClient = useQueryClient();
  return useMutation((data: Partial<FileUploadResponse>) => patchFileData(fileId, data), {
...
async function getFileData(fileId: string) {
  const response = await api.get<FileUploadResponse>(`/system/attachments/${fileId}`);
  return response.data;
}
